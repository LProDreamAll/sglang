...
- **Low-Latency** — bs=1 single-stream chat. MTP 3/4 (largest decode acceleration), small chunked-prefill batches. Pick when interactive latency matters most.
- **Balanced** — moderate concurrency with decent latency. DP-attention + DeepEP, MTP 1/2 (gentler). Pick for typical multi-user serving.
- **Max-Throughput** — high-concurrency batch serving. MTP off (verify step costs more than it saves at saturation). Pick when you're saturating the GPUs.
...
For how to actually launch the image, see [Install → Method 3: Using Docker](../../../docs/get-started/install#method-3-using-docker). A minimal example (substitute the inner `sglang serve ...` with whatever the [command generator](#deploy) below produces):
...
- **Model Deployment Command:** see the [command panel above](#deploy).
